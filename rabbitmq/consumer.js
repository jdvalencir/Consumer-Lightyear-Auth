...
let serviceAccount = path.resolve(
  process.env.FIREBASE_API_KEY || "./config/serviceAccountKey.json"
);
    ...
    try {
      ...
      this.channel.consume(queue, async (msg) => {
        ...
        try {
          ...
          const { email, firstName, secondName, lastName, secondLastName } =
            data;
          ...
          logger.info(
            `Received message email: ${email}, fullName: ${fullName}`
          );
          ...
          logger.info(
            `Response from validate citizen: ${responseValidateCitizen.status}`
          );
          ...
          if (responseValidateCitizen.status === 204) {
            ...
            logger.error(
              `Citizen is not valid: ${responseValidateCitizen.status} - ${responseValidateCitizen.statusText}`
            );
            throw new Error(
              `Citizen is not valid: ${responseValidateCitizen.status} - ${responseValidateCitizen.statusText}`
            );
  ...
  async consumeRegisterCitizen(queue) {
    try {
      ...
      this.channel.consume(queue, async (msg) => {
        ...
        try {
          const data = JSON.parse(msg.content.toString());
          const { id, citizenName, citizenEmail, urlDocuments, confirmAPI } =
            data;

          logger.info(
            `Received message to register citizen: id=${id}, name=${citizenName}, email=${citizenEmail}`
          );
          ...
          try {
            resetUrl = await admin
              .auth()
              .generatePasswordResetLink(citizenEmail);
            logger.info(`Password reset link generated: ${resetUrl}`);
            logger.info(`Password reset email sent to ${citizenEmail}`);
          } catch (error) {
            logger.error(
              `Error sending password reset email to ${citizenEmail}:`,
              error
            );
          }
          ...
          logger.info(
            `Payload forwarded to receive_data_transferred_docs_queue`
          );
